...
def F_output(stepName):
    ...
    # # Contact Pair field output (CP - CSTRESS, CDISP, CFORCE)
    # abqModel.FieldOutputRequest(name='CF_Output_R_V', 
    #                             createStepName=stepName, 
    ...
    # # Contact Pair (Bond) field output (CP - CSTRESS, CDISP, CFORCE)
    # abqModel.FieldOutputRequest(name='CF_Output_BOND', 
    #                             createStepName=stepName, 
    #                             variables=('CSTRESS', 'CDISP', 'CFORCE'), 
    #                             interactions=('CP_BOND', ), 
    #                             sectionPoints=DEFAULT, 
    #                             rebar=EXCLUDE)
...
def H_output(stepName):
    ...
    # # Define specific CFN between the vitreous and retina
    # regionDef=a.allInstances['V-1'].sets['V_R_Set']
    # abqModel.HistoryOutputRequest(name='Contact-VR-Hist', 
    #                               createStepName=stepName, 
    #                               variables=('CFN1', 'CFN2', 'CFN3', 'CAREA'), 
    #                               region=regionDef, 
    #                               sectionPoints=DEFAULT, 
    #                               rebar=EXCLUDE)
    
    # # Define specific CFN between the vitreous and retina
    # abqModel.HistoryOutputRequest(name='Contact_CP-R-V', 
    #                               createStepName=stepName, 
    #                               variables=('CFN1', 'CFN2', 'CFN3', 'CAREA'), 
    #                               interactions=('CP-R-V', ), 
    #                               sectionPoints=DEFAULT, 
    ...
    # # Define specific CFN between the retina and glue
    # abqModel.HistoryOutputRequest(name='Contact_CP-R-G', 
    #                               createStepName=stepName, 
    #                               variables=('CFN1', 'CFN2', 'CFN3', 'CAREA'), 
    #                               interactions=('CP-R-G', ), 
    #                               sectionPoints=DEFAULT, 
...
def CP_RV():
    abqModel.ContactProperty('CP_R_V_Int_Prop')
    a = abqModel.rootAssembly
    s1 = a.instances['R-1'].faces
    side1Faces1 = s1.getSequenceFromMask(mask=('[#4 ]', ), )
    a.Surface(side1Faces=side1Faces1, name='CP-R_V')
    region1=a.surfaces['CP-R_V']
    s1 = a.instances['V-1'].faces
    side1Faces1 = s1.getSequenceFromMask(mask=('[#5090 ]', ), )
    a.Surface(side1Faces=side1Faces1, name='CP-V_R')
    region2=a.surfaces['CP-V_R']
    abqModel.SurfaceToSurfaceContactExp(name='CP-R-V', 
                                        createStepName='Initial', 
                                        master=region1, 
                                        slave=region2, 
                                        sliding=FINITE, 
                                        interactionProperty='CP_R_V_Int_Prop', 
                                        weightingFactorType=SPECIFIED, 
                                        weightingFactor=1.0, 
...
def Amp():
    abqModel.SmoothStepAmplitude(name='TD_amp', 
                                 timeSpan=STEP, 
                                 data=((0.0, 0.0), 
                                       (30.0, 2e-05))
                                 )
    abqModel.SmoothStepAmplitude(name='omega', 
                                 timeSpan=STEP, 
                                 data=((0.0, 0.0), 
                                       (30.0, 0.000909174))
                                 )
...
def keywordBlockR_G_SET_NodeNum():
    # Work here because the abaqus default is to use Nset Generate  
    # Think about this method.  Used to be V_R_Set
    a = abqModel
    ...
    # Search the modelkwb for the desired insertion point.  If it is found, we 
    ...
    for n, line in enumerate(modelkwb.sieBlocks):
        if line.find('*Nset, nset=R_G_Set') >= 0:
            line_num = n
            break
    if line_num:
        line = line.replace('\n', ',') # replaces the new line with commas
        
        if line.find('generate') == -1:
            if line[-1] == ',':
                # if the node list ends with a comma
                # split up the string into ints
                nList = [int(i) for i in line[20:-1].split(',')]
            else:
                # split up the string into ints
                nList = [int(i) for i in line[20:].split(',')]
                
            nodeNum = len(nList) # count the number of nodes
        else:
            # use the equation in ABQ documentation to determine the number 
            # of nodes in the set
            # nNodes = (n2-n1)/increment
            # split up the string into ints
            nList = [int(i) for i in line[30:].split(',')]
            n1 = nList[0] # first node
            n2 = nList[1] # last node
            increment = nList[2]
            nodeNum = (n2 - n1)/increment + 1
        print(nodeNum, 'nodes in nset=R_G_Set')
    else:
        e = ("Error: R_G_Set was not found in the Model KeywordBlock to " + 
             "determine the number of nodes in the nodeSet.")
        raise Exception(" ".join(e))
    return nodeNum


def keywordBlockBond(MSTI, scaleFactor, FN, FS, db, ufn, ufs):
    a = abqModel
    modelkwb = a.keywordBlock
    assembly = a.rootAssembly
    ...
    # The abaqus keyword "*BOND" needs to be defined after the "*STEP" keyword
    # in the .inp file.  "Search for the "*Fixed Mass Scaling ..." term to 
    # add text specific to the bond
    keywordSearch = ('*Fixed Mass Scaling, ' + 
                     'dt={}, type=below min, '.format(MSTI) + 
                     'factor={:.0f}.'.format(scaleFactor))
    line_num = 0
    for n, line in enumerate(modelkwb.sieBlocks):
        if line == keywordSearch:
            line_num = n
            break
    if line_num:
        kwds = ('*Contact Pair, interaction=Bond_Int_Prop, mechanical ' + 
                'constraint=KINEMATIC, weight=0., ' + 
                'cpset=CP_Bond\nV-1.V_R_SURF_BOND, ' + 
                'R-1.R_V_SURF_BOND\n*Surface Interaction, ' + 
                'name=Bond_Int_Prop\n' + 
        ...
        e = ("Error: Mass Scaling was not found in the Model KeywordBlock.")
        raise Exception(" ".join(e))


def keywordBlockBondNPDFM(FN, FS):
    a = abqModel
    modelkwb = a.keywordBlock
    assembly = a.rootAssembly
    ...
    line_num = 0
    for n, line in enumerate(modelkwb.sieBlocks):
        # Use this line because CP from defined in code doesn't seem to work..
        if line == '*Contact Property Assignment\n ,  , IntProp-GC': 
            line_num = n
            break
    if line_num:
        kwds = ('*Contact Pair, interaction=Bond_Int_Prop, mechanical ' + 
...
def keywordBlockBondHistOutput():
    a = abqModel
    modelkwb = a.keywordBlock
    assembly = a.rootAssembly
...
def FEA():
    ...
    if tieInterface == True:
        """ If The VR interface is tied """
        VR_Tie()
    else:
        """ If bonding is true, then add the *Bond info to the .inp file """
          # determine number of nodes in the nodeSet
        nodeNum = keywordBlockR_G_SET_NodeNum()
        
        # Bonding
        # divide the bond tension load by the number of nodes in the set
        FN_Norm = FN/nodeNum
        # divide the bond shear load by the number of nodes in the set
        FS_Norm = FS/nodeNum
        
        if PDFMStatus == True:
            # Include post damage failure model
            # FN, FS, SpotWeldRadius, ufn, ufs
            keywordBlockBond(MSTI, scaleFactor, FN_Norm, 
                             FS_Norm, db, ufn, ufs)
        
        else:
            # Do not include post damage failure model
            keywordBlockBondNPDFM(FN_Norm, FS_Norm)
            
        keywordBlockBondHistOutput()
        keywordBlockBondFieldOutput()
...
if __name__ == '__main__':
    ...
    # gravity =              eval('False') # gravity
    # symmetry =             eval('False') # symmetry
    # simplified =           eval('True') # simplified model (not used anymore)
    # modelName =                 'ABQScript' # model name
    # jobName =                   'jobNameTest' # file name/job name
    # time =                float('50')
    ...
    # Strip the brackets from the string
    FNStrip = str(FN)[1:-1] 
    FNList = [float(i) for i in FNStrip.split(',')]
    FN = FNList[2] # value
    
    # Strip the brackets from the string
    FSStrip = str(FS)[1:-1]
    FSList = [float(i) for i in FSStrip.split(',')]
    FS = FSList[2] # value
    
    dbStrip = str(db)[1:-1] # Strip the brackets from the string
    dbList = [float(i) for i in dbStrip.split(',')]
    db = dbList[2] # value
    
    ufnStrip = str(ufn)[1:-1] # Strip the brackets from the string
    ufnList = [float(i) for i in ufnStrip.split(',')]
    ufn = ufnList[2] # value
    
    ufsStrip = str(ufs)[1:-1] # Strip the brackets from the string
    ufsList = [float(i) for i in ufsStrip.split(',')]
    ufs = ufsList[2] # value
    
    """ Write the FEA Code """
    Mdb()
    modelDescription = ('Measure adhesion between the retina & vitreous of ' + 
                        'the human eye using the Abaqus *Bond technique')
